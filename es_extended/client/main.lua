...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	if Config.EnableHud then
		...
		local orgTpl = '<div>{{org_label}}{{grade_label}}</div>'

		local gradeLabel = ESX.PlayerData.org.grade_label ~= ESX.PlayerData.org.label and ESX.PlayerData.org.grade_label or ''
		if gradeLabel ~= '' then gradeLabel = ' - '..gradeLabel end

		ESX.UI.HUD.RegisterElement('org', #ESX.PlayerData.accounts, 0, orgTpl, {
			org_label = ESX.PlayerData.org.label,
			grade_label = gradeLabel
		})
	end
...
AddEventHandler('esx:setOrg', function(Org)
	if Config.EnableHud then
		local gradeLabel = Org.grade_label ~= Org.label and Org.grade_label or ''
		if gradeLabel ~= '' then gradeLabel = ' - '..gradeLabel end
