...
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`,`org`, `org_grade`, `group`, `position`, `inventory`, `skin`, `loadout`'
...
if Config.Multichar then
	AddEventHandler('esx:onPlayerJoined', function(src, char, data)
		while not next(ESX.Jobs) and not next(ESX.Orgs) do Wait(50) end
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	local org, grade2, orgObject, grade2Object = result.org, tostring(result.org_grade)
	...
	-- Org
			if ESX.DoesOrgExist(org, grade) then
				orgObject, grade2Object = ESX.Orgs[org], ESX.Orgs[org].grades[grade2]
			else
				print(('[^3WARNING^7] Ignoring invalid org for %s [org: %s, grade: %s]'):format(identifier, org, grade))
				org, grade2 = 'unemployed2', '0'
				orgObject, grade2Object = ESX.Orgs[org], ESX.Orgs[org].grades[grade2]
			end
			...
			userData.org.grade = tonumber(grade2)
			userData.org.grade_name = grade2Object.name
			userData.org.grade_label = grade2Object.label
			--userData.org.grade_salary = gradeObject.salary

			userData.org.skin_male = {}
			userData.org.skin_female = {}

			if gradeObject.skin_male then userData.org.skin_male = json.decode(gradeObject.skin_male) end
			if gradeObject.skin_female then userData.org.skin_female = json.decode(gradeObject.skin_female) end
