...
function Core.SavePlayer(xPlayer, cb)
	MySQL.prepare('UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `org` = ?, `org_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?', {
...
function Core.SavePlayers(cb)
	...
	if count > 0 then
		...
		MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `org` = ?, `org_grade` = ?,  `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?", parameters,
		function(results)
			if results then
...
function ESX.GetExtendedPlayers(key, val)
	...
	for k, v in pairs(ESX.Players) do
		if key then
			...
			if (key == 'org' and v.org.name == val) or v[key] == val then
				xPlayers[#xPlayers + 1] = v
			end
		else
			xPlayers[#xPlayers + 1] = v
...
function ESX.GetIdentifier(playerId)
	for k,v in ipairs(GetPlayerIdentifiers(playerId)) do
		if string.match(v, 'steam:') then
			local identifier = v --string.gsub(v, 'steam:', '')
			return identifier
		end
	end
end
...
function ESX.DoesOrgExist(org, grade)
	grade = tostring(grade)

	if org and grade then
		if ESX.Orgs[org] and ESX.Orgs[org].grades[grade] then
			return true
		end
	end
