...
ESX.RegisterCommand('setorg', 'admin', function(xPlayer, args, showError)
	if ESX.DoesOrgExist(args.org, args.grade) then
		args.playerId.setOrg(args.org, args.grade)
	else
	...
	{name = 'grade', help = _U('command_setorg_grade'), type = 'number'}
}})
...
ESX.RegisterCommand('revive', 'admin', function(source, args, user)
	if args[1] ~= nil then
