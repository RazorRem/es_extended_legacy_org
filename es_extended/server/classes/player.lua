...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, org, loadout, name, coords)
	...
	function self.setOrg(org, grade)
		grade = tostring(grade)
		local lastOrg = json.decode(json.encode(self.org))

		if ESX.DoesOrgExist(org, grade) then
			local orgObject, gradeObject = ESX.Orgs[org], ESX.Orgs[org].grades[grade]
			...
			self.org.grade_name   = gradeObject.name
			self.org.grade_label  = gradeObject.label
			self.org.grade_salary = gradeObject.salary
